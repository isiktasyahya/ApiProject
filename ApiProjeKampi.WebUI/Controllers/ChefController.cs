...
namespace ApiProjeKampi.WebUI.Controllers
{
    public class ChefController : Controller
    {
        ...
        public async Task<IActionResult> ChefList()
        {
            ...
            var responseMessage = await client.GetAsync("https://localhost:7063/api/Chefs");
        ...
        public async Task<IActionResult> CreateChef(CreateChefDto createChefDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7063/api/Chefs", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            await client.DeleteAsync("https://localhost:7063/api/Chefs?id=" + id);
            return RedirectToAction("ChefList");
        }
        [HttpGet]
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync("https://localhost:7063/api/Chefs/GetChef?id=" + id);
            var jsonData = await responseMessage.Content.ReadAsStringAsync();
            var values = JsonConvert.DeserializeObject<GetChefByIdDto>(jsonData);
            return View(values);
        }
        [HttpPost]
        public async Task<IActionResult> UpdateChef(UpdateChefDto updateChefDto)
        {
            ...
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync("https://localhost:7063/api/Chefs", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("ChefList");
            }
